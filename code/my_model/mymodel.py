...
class MyModel(nn.Module):
  ...
  def forward(self, x_atom, x_bond, x_atom_index, x_bond_index, x_mask, x_chemical_info):
    ## AttentiveFP
...
#             print(context.shape)
    ...
    for d in range(self.radius-1):
        ...
        neighbor_feature = [activated_features[i][x_atom_index[i]] for i in range(batch_size)]
...
class ChemInfo(nn.Module):
  ...
  def forward(self, x_atom, x_bonds, x_atom_index, x_bond_index, x_mask, x_chemical_info):
    ## ChemicalInfo
    chemical_info = F.relu(self.fc1(x_chemical_info))
    chemical_info = self.bn1(chemical_info)
