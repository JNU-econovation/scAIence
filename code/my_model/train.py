...
config = {
        "ChemInfo_vec": tune.sample_from(lambda _: np.random.randint(10, 100)),
        "ChemInfo_output_dim": tune.sample_from(lambda _: np.random.randint(4, 32)),
        "lr": tune.loguniform(1e-4, 1e-1),
        "batch_size": tune.choice([2, 4, 8, 16, 32, 64, 128]),
        "AttentiveFP_output_dim": tune.sample_from(lambda _: np.random.randint(10, 100))
    }

def train(model, dataset, optimizer, loss_function, config):
        model.train()
        valList = np.arange(0,dataset.shape[0])
        np.random.shuffle(valList)
        batch_list = []

        for i in range(0, dataset.shape[0], config['batch_size']):
            batch = valList[i:i+config['batch_size']]
        ...
        for counter, train_batch in enumerate(batch_list):
            ...
            x_chemical_info = total_chem_array(smiles_list, vec_len=50)
...
models = ["ChemInfo", "AttentiveFP","my_model"]

for i in range(20):
    kargs = {'cheminfo_input_dim' : 58,
        'cheminfo_output_dim' : config['ChemInfo_output_dim'],
        'radius' : 2,
        'T' : 2,
        'input_feature_dim' : 39,
        'input_bond_dim' : 10,
        'fingerprint_dim' : 64,
        'output_units_num' : config['AttentiveFP_output_dim'],
        'p_dropout' : 0.2
    }
    ...
    result = tune.run(
        partial(train, dataset=train_df),
    ...
    for model_name in models:
        ...
        optimizer = optim.Adam(model.parameters(), config['lr'], weight_decay=10**-weight_decay)
        ...
        for epoch in range(800):
            ...
            train_MAE, train_MSE, train_CROSS_ENTROPY, train_r2, train_acc, train_pre, train_re, train_auprc, train_auroc = eval(model, train_df, config)
            test_MAE, test_MSE, test_CROSS_ENTROPY, test_r2, test_acc, test_pre, test_re, test_auprc, test_auroc = eval(model, test_df, config)
            ...
            tune.report(loss=(test_CROSS_ENTROPY), accuracy=test_acc)
        
            train(model, train_df, optimizer, loss_function, config)
            end_time = time.time()
